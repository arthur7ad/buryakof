...
namespace app\components\ipdata;
...
class IpGeoBase extends Component {
    ...
    protected function generateCityTables($zip) {
        ...
        Yii::$app->{$this->db}->createCommand()->batchInsert(
                self::DB_CITY_TABLE_NAME, ['id', 'name', 'region_id', 'latitude', 'longitude'], $cities
        )->execute();
        ...
        Yii::$app->{$this->db}->createCommand()->batchInsert(
                self::DB_REGION_TABLE_NAME, ['id', 'name'], $regions
        )->execute();
    }
    ...
    protected function generateIpTable($zip) {
        ...
        foreach ($ipsArray as $ip) {
            ...
            if ($i === self::DB_IP_INSERTING_ROWS) {
                Yii::$app->{$this->db}->createCommand()->batchInsert(
                        self::DB_IP_TABLE_NAME, ['ip_begin', 'ip_end', 'country_code', 'city_id'], $values
                )->execute();
